            ...
            <td>
              <div class="flex items-center">
                ...
                <c:if test="${ actorCanCancelBadReaction }">
                  <a onclick="alert(this.title); return false;" title="먼저 싫어요를 취소해주세요."
                    href="/usr/reactionPoint/doGoodReaction?relTypeCode=article&relId=${param.id}&replaceUri=${rq.encodedCurrentUri}"
                    class="btn btn-xs btn-primary btn-outline"> 좋아요 👍 </a>
                  <span>&nbsp;</span>
                  <a
    ...
    <table class="table table-fixed w-full">
      ...
      <tbody>
        <c:forEach var="reply" items="${ replies }">
          <tr class="table-1">
            <th class="table-1">${ reply.id }</th>
            <td class="table-1">${ reply.forPrintType1RegDate }</td>
            <td class="table-1">${ reply.forPrintType1UpdateDate }</td>
            <td class="table-1">${ reply.goodReactionPoint }</td>
            <td class="table-1">${ reply.extra__writerName }</td>
            <td class="table-1">
